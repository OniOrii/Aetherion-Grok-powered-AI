...
- **Hunt animal progression (OwO feel-parity):** pets still start at **level 1 / 0 XP** (`level_of(0)==1`). Level curve now matches OwO `animalUtil.js`: XP to leave level `L` is **`L^4 + 1000`** (was flat `XP_PER_LEVEL=25`). Effective **LEVEL_CAP is 50** in base and ext (base was 20). Hunt team XP uses OwO Manual Hunting / `animal.json` amounts for C–M: **Common 1, Uncommon 10, Rare 20, Epic 400, Mythical 1000** (replaces ext `{8,12,20,40,80}`). Per OwO source, **team pets only** gain that hunt XP — the newly caught animal does not unless it is already on the team. `/hunt` still returns `xp_gain` so the catch line can show `| emoji gained **Nxp**!`. Battle base XP stays **200 / 100 / 50** (base no longer uses 40/25/15); ext win bonuses use OwO source streak + float-avg level-diff (`round(600 * diff)`). Stats still scale with `stats_for` (HP +6 / ATK +2 per level). Catalog unchanged in this PR.
