...
def register_blackjack(tree, is_guild_allowed) -> None:
    ...
    @tree.command(name="daily", description="Claim today's Aether Coins and Hunt supplies.")
    async def daily_slash(interaction: discord.Interaction):
        ...
        hunt_got = {"lootbox": 0, "crate": 0, "raid_ticket": 0}
        if creator_is_author(interaction.user.id):
            try:
                hunt_got = hunt.grant_daily_supplies(interaction.user.id) or hunt_got
            except Exception:
                logger.exception("daily hunt supplies failed user=%s", interaction.user.id)
        lb = int(hunt_got.get("lootbox") or 0)
        cr = int(hunt_got.get("crate") or 0)
        tk = int(hunt_got.get("raid_ticket") or 0)
        hunt_any = lb > 0 or cr > 0 or tk > 0
        if already and not hunt_any:
            embed = discord.Embed(
                title="\u2726 Daily",
        ...
        lines = []
        if granted:
            lines.append(f"Claimed {ai_coins.coins(f'**{granted:,}**')}.")
        if hunt_any:
            parts = []
            if lb:
                parts.append(f"**{lb}** lootbox{'es' if lb != 1 else ''}")
            if cr:
                parts.append(f"**{cr}** weapon crate{'s' if cr != 1 else ''}")
            if tk:
                parts.append(f"**{tk}** raid ticket{'s' if tk != 1 else ''}")
            lines.append("Hunt supplies: " + ", ".join(parts) + ".")
        lines.append(f"Wallet: {ai_coins.coins(f'**{bal:,}**')}.")
        embed = discord.Embed(
            title="\u2726 Daily claimed",
            description="\n".join(lines),
            color=EMBED_WIN,
