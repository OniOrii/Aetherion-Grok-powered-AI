...
def _battle_embed(name: str, result: dict, frame: dict, *, final: bool) -> discord.Embed:
    ...
    desc = board.result_caption(result) if final else "\n".join((frame.get("lines") or [])[-3:])
    embed = discord.Embed(color=color, description=desc or None)
    embed.set_author(name=f"{name} goes into battle!")
    ...
    if final:
        ...
        if result.get("payout"):
            extra.append(ai_coins.won_line(int(result["payout"])))
        pocket = int(result.get("balance") or 0)
        extra.append("Pocket \u00b7 " + ai_coins.coins(f"**{pocket:,}**"))
        extra.append(f"streak {int(result.get('streak') or 0)}")
        embed.set_footer(text=" \u00b7 ".join(extra) + f"\n{hunt.WIP_FOOTER}")
    else:
        embed.set_footer(text=f"Turn {frame.get('turn') or 1} / 5 \u00b7 {hunt.WIP_FOOTER}")
    embed.set_image(url="attachment://battle.png")
    return embed


def _battle_file(frame: dict) -> discord.File:
...
def register_hunt(tree, is_guild_allowed) -> None:
    ...
    async def battle_slash(interaction: discord.Interaction):
        ...
        if not frames:
            frames = [{
                "turn": max(1, int(result.get("rounds") or 1)),
        ...
        try:
            ...
            msg = await interaction.followup.send(
                embed=_battle_embed(name, result, first, final=len(frames) == 1),
                file=_battle_file(first),
                wait=True,
            )
            for index, frame in enumerate(frames[1:], start=1):
                ...
                await msg.edit(
                    embed=_battle_embed(name, result, frame, final=final),
                    attachments=[_battle_file(frame)],
                )
        except Exception:
            ...
            pocket = ai_coins.coins(f"**{int(result.get('balance') or 0):,}**")
