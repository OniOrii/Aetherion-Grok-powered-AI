...
ANIMALS: tuple[tuple[str, str, str, str], ...] = (
    ...
    ("clay_crab", "Clay Crab", "\U0001f980", COMMON),
    ("weed_sprite", "Weed Sprite", "\U0001f331", COMMON),
...
def resolve_animal(query: str | None) -> str | None:
    ...
    hits = [
        aid
        for aid, name, _emoji, _rar in ANIMALS
        if text in aid or text in name.lower()
    ]
    if len(hits) == 1:
        return hits[0]
    return None
...
def roll_animal(rng: random.Random | None = None) -> str:
    ...
    rarity = rng.choices(
        ...
        weights=[item[1] for item in buckets],
        k=1,
    )[0]
    pool = [aid for aid, _n, _e, rar in ANIMALS if rar == rarity]
...
def cooldown_left(row: dict[str, Any], now: float | None = None) -> float:
    ...
    last = float(row.get("last_hunt") or 0)
    return max(0.0, HUNT_COOLDOWN - (now - last))


def hunt(user_id: int, rng: random.Random | None = None) -> dict[str, Any]:
    """Spend HUNT_COST coins, roll one animal, give light team XP."""
    rng = rng or random.Random()
    with _lock:
        store = _load_store()
        ...
        if pocket < HUNT_COST:
            return {
                "ok": False,
                "error": f"Hunt costs {ai_coins.coins(HUNT_COST)}. You have {ai_coins.coins(pocket)}.",
            }
...
def snapshot(user_id: int) -> dict[str, Any]:
    with _lock:
        ...
        return {
            ...
            "team": list(row["team"]),
            "xp": dict(row["xp"]),
            "last_hunt": float(row["last_hunt"]),
            "cooldown": cooldown_left(row),
        }
    ...
    with _lock:
        ...
        if have - count < keep:
            if on_team:
                return {
                    "ok": False,
                    "error": f"{animal_label(animal_id)} is on your team. Keep at least one, or take it off the team first.",
                }
            if have <= 0:
...
def simulate_battle(
    ...
    while _living(player) and _living(enemy) and rounds < 12:
        ...
        for attacker in order:
            ...
            log.append(
                f"{animal_label(attacker['id'])} hits {animal_label(target['id'])} for {dmg}. {mark}."
            )
...
def zoo_lines(zoo: dict[str, int], xp: dict[str, int]) -> list[str]:
    ...
    for rarity in RARITY_ORDER:
        ...
        for aid, count in chunk:
            ...
            lines.append(f"{animal_label(aid)} x{count} \u00b7 Lv {lvl}")
    return lines
...
def team_lines(team: list[str | None], xp: dict[str, int], zoo: dict[str, int]) -> list[str]:
    ...
    for i in range(TEAM_SIZE):
        ...
        lines.append(
            f"**{i + 1}.** {animal_label(aid)} \u00b7 Lv {lvl} \u00b7 {hp} HP / {atk} ATK \u00b7 owned {have}"
        )
    return lines
...
def fighter_line(pet: dict[str, Any]) -> str:
    return (
        f"{animal_label(pet['id'])} Lv {pet['level']} "
        f"({max(0, pet['hp'])}/{pet['max_hp']})"
    )
