...
def attach_listeners(client):
    if client is None:
        return
    hooked = getattr(client, "_aetherion_log_events", None)
    if hooked is None:
        hooked = set()
        client._aetherion_log_events = hooked
    ...
    for name, fn in pairs:
        ...
        if existing is fn or name in hooked:
            hooked.add(name)
            continue
            ...
            def _bind(old, new, ev):
                async def _wrapped(*args, **kwargs):
                    try:
                        ...
                        logger.exception("server log %s failed", ev)
        ...
        hooked.add(name)
    logger.info("server log listeners attached (%s events)", len(hooked))
