...
def _voice_model() -> str:
    return resolve_voice_model(
        getattr(settings, 'voice_grok_model', None),
        getattr(settings, 'grok_model', None),
    )

def _voice_effort() -> str:
    return resolve_voice_effort(getattr(settings, 'voice_reasoning_effort', None))

def _voice_cap() -> int:
    return resolve_voice_max_tokens(getattr(settings, 'voice_max_output_tokens', None))
...
class VoiceSession:
    ...
    async def _reply(self, pcm16: bytes) -> None:
        try:
            ...
            music = await handle_music(self._vc, prompt)
            if music is not None:
                speak = _speakable(music.get('speak') or '')
                if speak:
                    logger.info('music: %s', speak[:200])
                    audio = await self._tts(speak)
    ...
    async def _grok_text(self, heard: str) -> str:
        ...
        try:
            ...
            user_content = heard
        model = _voice_model()
        effort = _voice_effort()
        cap = _voice_cap()
        logger.info('voice grok model=%s effort=%s cap=%s', model, effort, cap)
        headers = {'Authorization': f'Bearer {key}', 'Content-Type': 'application/json'}
        async with httpx.AsyncClient(timeout=90) as client:
            for include_reasoning in (True, False):
                payload = build_voice_responses_payload(
                    system, user_content, model=model, effort=effort,
                    max_output_tokens=cap, include_reasoning=include_reasoning,
                )
                resp = await client.post('https://api.x.ai/v1/responses', headers=headers, json=payload)
                if resp.status_code < 400:
                    text = _extract_response_text(resp.json())
                    if text:
                        return text
                if resp.status_code not in (400, 422) and include_reasoning:
                    logger.warning('voice responses %s: %s', resp.status_code, (resp.text or '')[:240])
                    break
                if include_reasoning:
                    logger.info('voice responses rejected reasoning; retrying without it')
            for include_reasoning in (True, False):
                ...
                if resp.status_code < 400:
                    ...
                    return (msg or '').strip()
                if resp.status_code not in (400, 422) and include_reasoning:
                    logger.warning('voice chat %s: %s', resp.status_code, (resp.text or '')[:240])
                    break
                if include_reasoning:
                    logger.info('voice chat rejected reasoning; retrying without it')
            return ''
